...
async function waitForExperienceReady(page: Page) {
  await expect(page.locator(".portfolio-route")).toBeVisible();
...
const localeCases = [
  {
    ...
    labels: {
      ...
      sidebarNavLabel: "Section navigation",
  ...
  },
  {
    ...
    labels: {
      ...
      sidebarNavLabel: "Navegación de secciones",
...
test.describe("portfolio i18n interaction and accessibility", () => {
  for (const localeCase of localeCases) {
    test(`${localeCase.locale}: canonical homepage renders the portfolio experience and locale switch`, async ({
      page,
    }) => {
    ...
    test(`${localeCase.locale}: portfolio tokens drive key surfaces and text contrast is compliant`, async ({
      page,
    }) => {
      await page.goto(localeCase.path);
      await waitForExperienceReady(page);
      ...
      const tokenValues = await page.evaluate(() => {
        ...
        return {
          bg: rootStyle.getPropertyValue("--portfolio-color-bg").trim(),
        };
      });

      const routeBackground = await page
        .locator(".portfolio-route")
        .evaluate((node) => {
  ...
  test("reduced-motion token collapses duration to 0ms", async ({
    ...
    const motionDuration = await page.evaluate(() => {
      return window
        ...
        .getPropertyValue("--portfolio-motion-duration")
        .trim();
    });
