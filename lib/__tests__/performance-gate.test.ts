...
function createMetricsFixture(routes: Record<string, { lcp: number; cls: number; inp: number }>) {
...
describe('v2 performance gate script', () => {
  it('passes when the canonical portfolio routes stay within budget', () => {
    ...
    expect(output).toContain('Performance gate passed for canonical portfolio routes')
  })
