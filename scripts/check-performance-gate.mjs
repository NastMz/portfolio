...
const metricsPath = process.env.PORTFOLIO_PERF_METRICS_PATH
  ? resolve(process.cwd(), process.env.PORTFOLIO_PERF_METRICS_PATH)
  : resolve(process.cwd(), "reports/performance/portfolio-route-metrics.json");
...
try {
  ...
  throw new Error(
    `Performance gate failed: cannot read metrics file at ${metricsPath}`,
  );
}
...
if (JSON.stringify(routeKeys) !== JSON.stringify(expectedRouteKeys)) {
  failures.push(
    `route alignment mismatch: expected ${expectedRouteKeys.join(", ")} but received ${routeKeys.join(", ") || "none"}`,
  );
}
...
for (const route of requiredRoutes) {
  ...
  if (typeof metric.lcp !== "number" || metric.lcp > thresholds.lcp) {
    failures.push(
      `${route}: LCP ${metric.lcp ?? "N/A"}ms exceeds ${thresholds.lcp}ms`,
    );
  }

  if (typeof metric.cls !== "number" || metric.cls >= thresholds.cls) {
    failures.push(
      `${route}: CLS ${metric.cls ?? "N/A"} must be < ${thresholds.cls}`,
    );
  }

  if (typeof metric.inp !== "number" || metric.inp > thresholds.inp) {
    failures.push(
      `${route}: INP ${metric.inp ?? "N/A"}ms exceeds ${thresholds.inp}ms`,
    );
  }
}
