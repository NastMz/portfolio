...
vi.mock('next-intl/middleware', () => ({
  default: vi.fn(() => vi.fn((request: NextRequest) => NextResponse.next({ request: { headers: request.headers } }))),
}))
...
describe('document language propagation', () => {
  it('sets the locale header in middleware for supported locale segments', () => {
    ...
    expect(englishResponse.headers.get('x-middleware-request-x-locale')).toBe('en')
    expect(spanishResponse.headers.get('x-middleware-request-x-locale')).toBe('es')
  })

  it('keeps the default entry locale for the bare root path', () => {
    const response = appendLocaleHeader(NextResponse.next(), new NextRequest('https://example.com/'))
