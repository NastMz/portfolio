...
interface SystemStateDriftProps {
  initialState: string;
  states?: string[];
...
export function SystemStateDrift({
  initialState,
  states,
  minIntervalMs = DEFAULT_MIN_INTERVAL_MS,
  intervalWindowMs = DEFAULT_INTERVAL_WINDOW_MS,
}: SystemStateDriftProps) {
  ...
  const reservedWidth = useMemo(() => {
    const longestState = availableStates.reduce(
