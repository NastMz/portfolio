...
import { SidebarRotatingLog } from "./SidebarRotatingLog";
...
function renderNavIcon(href: NavAnchor, className = "h-5 w-5") {
  if (href === "#overview") {
    return (
      <svg
        aria-hidden="true"
  ...
  if (href === "#systems") {
    return (
      ...
      >
        ...
        />
        <path
          d="M6 6v8c0 1.4 2.7 2.5 6 2.5s6-1.1 6-2.5V6"
          stroke="currentColor"
          strokeWidth="1.6"
        />
        <path
          d="M6 10c0 1.4 2.7 2.5 6 2.5s6-1.1 6-2.5"
          stroke="currentColor"
          strokeWidth="1.6"
        />
      </svg>
  ...
  if (href === "#stack") {
    return (
      ...
      >
        ...
        />
        <path
          d="M9 5v3M15 5v3M9 16v3M15 16v3M4 10h2M4 14h2M18 10h2M18 14h2"
          stroke="currentColor"
          strokeLinecap="round"
          strokeWidth="1.6"
        />
...
export function SectionNavigation({
  ...
  const [activeHref, setActiveHref] = useState<NavAnchor>("#overview");
  ...
  useEffect(() => {
    const observedSections = copy.nav
      .map((item) => {
        ...
        return element ? { href: item.href as NavAnchor, element } : null;
      })
      .filter(
        (item): item is { href: NavAnchor; element: HTMLElement } =>
          item !== null,
      );
    ...
    const resolveActiveSection = () => {
      ...
      const viewportAnchor =
        window.scrollY + Math.max(window.innerHeight * 0.28, 180);
    ...
    const syncFromHash = () => {
      const hash = window.location.hash as NavAnchor;
  ...
  const activeItem =
    copy.nav.find((item) => item.href === activeHref) ?? copy.nav[0];
  const activeSectionLabel = activeItem
    ? `[SECTION: ${toSectionToken(activeItem.label)}]`
    : null;
  ...
  return (
    <>
      <aside className="bg-[#0b0b0b] text-[#FF7CF5] font-label text-[10px] uppercase tracking-wider fixed left-0 top-0 hidden h-full w-16 md:flex md:w-64 flex-col z-40 pt-20 border-r border-zinc-800/30">
        <div className="px-4 mb-8 hidden md:block">
          <div className="mb-5 border border-zinc-800 bg-black/40 px-3 py-3">
            <div className="text-[8px] tracking-[0.24em] text-zinc-500/95 mb-1 font-medium">
              [ACTIVE_PORTFOLIO]
            </div>
            <div className="font-headline text-sm text-primary tracking-tight">
              [{brandTitle.replace(/\s+/g, "_").toUpperCase()}]
            </div>
          </div>
          ...
          <div className="space-y-1.5 text-zinc-300/85">
            {copy.monitorItems.map((item) => (
              ...
              >
                ...
                <span
                  className={`${item.valueClassName ?? "text-white"} max-w-[11rem] text-right leading-tight`}
                >
                  {item.value}
                </span>
              </div>
          ...
          {copy.nav.map((item) => {
            ...
            return (
              <a
                ...
                title={item.label}
              >
                {renderNavIcon(item.href as NavAnchor)}
                {renderNavLabel(
                  item.label,
                  "text-[8px] leading-[1.08] text-center md:text-left md:text-[10px] md:leading-tight",
                  isActive
                    ? "text-primary/75 text-[7px] md:text-[9px]"
                    : "text-zinc-500/95 text-[7px] md:text-[9px] font-medium",
                )}
              </a>
            );
          })}
        ...
        <div className="mt-auto p-4 border-t border-zinc-800/20 bg-black/40">
          <div className="mb-4">
            <div className="text-zinc-400 text-[8px] mb-2">
              {copy.throughputTitle}
            </div>
            ...
            <SidebarRotatingLog
              initialLog={copy.throughputLatestLog}
              logs={copy.throughputLogs}
            />
          </div>
      ...
      >
        <div className="mb-2 px-1 font-label text-[9px] tracking-[0.18em] text-primary/80">
          {activeSectionLabel}
        </div>
            ...
            return (
              <li key={`${item.href}-${item.label}`}>
                <a
                  ...
                  title={item.label}
                >
                  {renderNavIcon(item.href as NavAnchor, "h-4 w-4")}
                  {renderNavLabel(
                    item.label,
                    "text-[7px] leading-none text-center",
                    isActive
                      ? "text-primary/70 text-[6px]"
                      : "text-zinc-500/95 text-[6px] font-medium",
                  )}
                </a>
              </li>
            );
