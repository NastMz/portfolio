...
interface SidebarRotatingLogProps {
  initialLog: string;
  logs?: string[];
}
...
export function getStableRotationsUntilSequence(
  randomValue: number = Math.random(),
) {
  return (
    MIN_STABLE_ROTATIONS_BETWEEN_RECOVERABLE_WARNINGS +
    Math.floor(randomValue * STABLE_ROTATION_VARIANCE)
  );
}
...
export function getNextSidebarLogState(
  currentState: SidebarLogState,
  availableLogs: string[],
): SidebarLogState {
  ...
  if (currentState.pendingSequence.length > 0) {
    ...
    return {
      ...
      stableRotationsUntilSequence:
        remainingSequence.length === 0
...
export function SidebarRotatingLog({
  initialLog,
  logs,
}: SidebarRotatingLogProps) {
  const availableLogs = useMemo(() => logs?.filter(Boolean) ?? [], [logs]);
  const [logState, setLogState] = useState<SidebarLogState>(() =>
    createInitialState(initialLog, availableLogs),
  );
  ...
  useEffect(() => {
    ...
    const scheduleNextTick = () => {
      timeoutId = window.setTimeout(() => {
        setLogState((currentState) =>
          getNextSidebarLogState(currentState, availableLogs),
        );
        scheduleNextTick();
      }, getNextDelay());
    };
  ...
  const currentLog =
    availableLogs.length <= 1
      ? (availableLogs[0] ?? initialLog)
      : logState.currentLog;

  return (
    <div
      aria-live="polite"
      className="text-[8px] text-zinc-500/95 mt-2 font-label font-medium transition-opacity duration-300 motion-reduce:transition-none"
    >
      {currentLog}
    </div>
  );
