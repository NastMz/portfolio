...
export interface ContactTerminalFormCopy {
  bootLine: string;
  helperLine: string;
  previewHint: string;
  fallbackHint: string;
...
export function ContactTerminalForm({ copy }: ContactTerminalFormProps) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = [
      `[${copy.mail.contextLabel}: -contact-handshake]`,
    ...
    const launchTimeoutId = window.setTimeout(
      ...
      SUCCESS_SEQUENCE_STEP_MS * (sequence.length - 1) +
        SUCCESS_SEQUENCE_COMPLETE_DELAY_MS,
    );
  ...
  async function handleCopyEndpoint() {
    try {
      ...
      setFeedback({
        type: "success",
        messages: ["> fallback_ready", "> endpoint_copied"],
      });
    } catch {
  ...
  return (
    <div className="border border-zinc-800 bg-[#080808] text-left shadow-[0_0_0_1px_rgba(255,124,245,0.06)]">
      <div className="flex items-center justify-between gap-4 border-b border-zinc-800 px-4 py-3">
        ...
        <div className="font-label text-[10px] uppercase tracking-[0.28em] text-zinc-500/95 font-medium">
          terminal://contact-protocol
        </div>
      </div>
      ...
      <div className="border-b border-zinc-800 px-4 py-3 font-label text-[10px] uppercase tracking-[0.2em] text-primary">
        ...
        <div className="mt-1 text-zinc-400/90 font-medium">
          {copy.helperLine}
        </div>
      </div>
      ...
      <form className="space-y-4 p-4 md:p-6" noValidate onSubmit={handleSubmit}>
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 group">
            <label
              className="block font-label text-[10px] text-zinc-400/90 tracking-widest uppercase mb-2"
              htmlFor="contact-identity"
            >
              {copy.identityLabel}
            </label>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-primary font-label">
                &gt;
              </span>
              <input
                id="contact-identity"
                aria-describedby={
                  fieldErrors.identity ? identityErrorId : undefined
                }
            ...
            {fieldErrors.identity ? (
              <p
                className="mt-2 font-label text-[10px] uppercase tracking-widest text-error"
                id={identityErrorId}
                role="alert"
              >
                [{fieldErrors.identity}]
          ...
          <div className="flex-1 group">
            ...
            <div className="relative">
              ...
              <input
                id="contact-endpoint"
                aria-describedby={
                  fieldErrors.endpointAddress ? endpointErrorId : undefined
                }
        ...
        <div className="space-y-2">
          <div className="font-label text-[10px] uppercase tracking-[0.22em] text-zinc-500/95 font-medium">
            {copy.previewHint}
          </div>
          <div className="border-l border-primary/20 bg-zinc-950/70 px-4 py-3 font-label text-[10px] uppercase tracking-[0.18em] text-zinc-400/90 font-medium cursor-default select-none">
            ...
            {identity.trim() || copy.identityPlaceholder}&quot;
            --endpoint=&quot;
            {endpointAddress.trim() || copy.endpointPlaceholder}&quot;
          </div>
        </div>
        ...
        {feedback ? (
          feedback.type === "error" ? (
            ...
            <div
              className="space-y-1 font-label text-[10px] uppercase tracking-widest text-primary"
              role="status"
              aria-live="polite"
            >
        ...
        <button
          className="w-full bg-primary text-on-primary py-5 font-headline font-bold text-xl glitch-hover disabled:cursor-not-allowed disabled:opacity-90"
        ...
        <div className="font-label text-[10px] uppercase tracking-[0.22em] text-zinc-500/95 text-center font-medium">
          {copy.submitHint}
        </div>
