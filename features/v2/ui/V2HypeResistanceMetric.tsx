...
interface V2HypeResistanceMetricProps {
  initialValue?: number
}
...
function getVariationMagnitude(randomValue: number = Math.random()) {
  return VARIATION_STEPS[Math.floor(randomValue * VARIATION_STEPS.length)] ?? VARIATION_STEPS[0]
}

function getVariationDirection(currentValue: number, randomValue: number = Math.random()) {
...
export function createInitialHypeResistanceSnapshot(initialValue: number = BASE_VALUE): HypeResistanceSnapshot {
...
export function getNextHypeResistanceSnapshot(
  ...
  if (currentSnapshot.status === 'RECOVERED') {
    const nominalValue = clampValue(Math.max(BASE_VALUE, currentSnapshot.value))
  ...
  const direction = getVariationDirection(currentSnapshot.value, directionRandom)
  const nextValue = clampValue(currentSnapshot.value + direction * magnitude)
  const thresholdStatus = getThresholdStatus(nextValue)

  if (currentSnapshot.previousStatus === 'DEGRADED' && thresholdStatus !== 'DEGRADED') {
    return {
      value: nextValue,
      status: 'RECOVERED',
      previousStatus: thresholdStatus,
...
export function V2HypeResistanceMetric({ initialValue = BASE_VALUE }: V2HypeResistanceMetricProps) {
  const [snapshot, setSnapshot] = useState(() => createInitialHypeResistanceSnapshot(initialValue))
  ...
  useEffect(() => {
    ...
    const scheduleNextTick = () => {
      timeoutId = window.setTimeout(() => {
        setSnapshot((currentSnapshot) => getNextHypeResistanceSnapshot(currentSnapshot))
        scheduleNextTick()
      }, getNextDelay())
    }
  ...
  const reservedWidth = useMemo(() => formatMetric({ value: 100, status: 'FLUCTUATING', previousStatus: 'FLUCTUATING' }).length + 1, [])

  return (
    <span className="inline-block min-w-0 tabular-nums" style={{ minWidth: `${reservedWidth}ch` }}>
      {formatMetric(snapshot)}
    </span>
  )
}

export { BASE_VALUE as HYPE_RESISTANCE_BASE_VALUE, MIN_INTERVAL_MS as HYPE_RESISTANCE_MIN_INTERVAL_MS, INTERVAL_WINDOW_MS as HYPE_RESISTANCE_INTERVAL_WINDOW_MS }
