...
function resolveTraceState(target: Element): TraceState {
  ...
  if (interactiveTarget.matches('input, select, textarea, summary, label[for]')) {
    return 'inspected'
  }
...
export function V2HoverTrace() {
  ...
  useEffect(() => {
    ...
    const runTrace = (target: HTMLElement) => {
      ...
      if (signature === lastSignatureRef.current && now - lastTimestampRef.current < RECENT_REPEAT_GUARD_MS) {
        return
      }
      ...
      if (reducedMotionQuery.matches) {
        timeoutRefs.current.push(
          ...
          window.setTimeout(() => setPhase('idle'), HOLD_DURATION_MS + FADE_DURATION_MS)
        )
      ...
      const typingStepMs = Math.max(22, Math.floor(TYPING_DURATION_MS / Math.max(totalCharacters, 1)))
      ...
      timeoutRefs.current.push(
        ...
        window.setTimeout(() => setPhase('fading'), TYPING_DURATION_MS + HOLD_DURATION_MS),
        window.setTimeout(() => setPhase('idle'), TYPING_DURATION_MS + HOLD_DURATION_MS + FADE_DURATION_MS)
      )
    }
  ...
  return (
    <div aria-hidden="true" className="v2-hover-trace" data-phase={phase}>
      <span className="v2-hover-trace__message">{phase === 'idle' ? '' : visibleMessage || message}</span>
    </div>
  )
}
